import { bounds, HEX_RADIUS, parcels } from "@/lib/parcels";
 ...
 *
 * Derived from the generated grid rather than written down, because both
 * numbers change the moment the parcel count or the body does, and a stale
 * constant here produces a reference that looks right and is wrong.
 */
const COL_STEP = 1.5 * HEX_RADIUS;
const ROW_STEP = Math.sqrt(3) * HEX_RADIUS;

export function gridRef(x: number, y: number): string {
  const col = Math.round((x - bounds[0]) / COL_STEP);
  const row = Math.round((bounds[3] - y) / ROW_STEP);
  return `X: ${col} Y: ${row}`;
}
