    ...
    <div className="flex h-full flex-col overflow-y-auto border-l border-rule bg-void/97 backdrop-blur-sm">
      <div className="flex items-start justify-between gap-4 border-b border-rule px-5 py-5">
        <div>
          ...
          <p className="type-data mt-2 text-chalk-soft">{parcel.feature}</p>
          <p className="type-data text-chalk-muted">
            {gridRef(parcel.x, parcel.y)} ·{" "}
            {parcel.terrain === "mare" ? "Basalt plain" : "Highland"} ·{" "}
            {parcel.side === "near" ? "Nearside" : "Farside"}
          </p>
        </div>
      ...
      {/*
        The one thing a lunar grid has that a terrestrial one does not.
        Eighteen parcels out of 999 contain a place something has actually
        landed, and that is a matter of record rather than a feature of the
        market — so it sits above the figures, not among them.
      */}
      {parcel.sites && (
        <div className="border-b border-rule bg-field-raised px-5 py-4">
          <Label className="text-signal">Landed here</Label>
          <ul className="mt-2 space-y-1">
            {parcel.sites.map((site) => (
              <li key={site.name} className="type-data text-chalk">
                {site.name}
                <span className="text-chalk-muted">
                  {" · "}
                  {site.operator} · {site.year}
                </span>
              </li>
            ))}
          </ul>
