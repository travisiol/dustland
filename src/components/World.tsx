...
import { PARCEL_AREA_KM2, parcels, type Parcel } from "@/lib/parcels";
...
export function World() {
  ...
  const pills = [
    ...
    {
      key: "Ground each",
      value: `${PARCEL_AREA_KM2.toLocaleString("en-US")} km²`,
    },
    {
      key: "Plots taken",
      ...
      {!selected && (
        <div className="pointer-events-none absolute inset-x-0 bottom-12 px-4 sm:px-8 lg:inset-y-0 lg:right-auto lg:flex lg:w-[46%] lg:items-center">
          <div className="pitch pointer-events-auto w-full max-w-[520px]">
            ...
            <h1 className="type-hero wordmark-stencil mt-4 text-chalk">
              Dustland
            </h1>
            <p className="type-display mt-4 text-signal">Claim a parcel</p>
            <p className="type-display text-chalk">of the Moon</p>

            <p className="type-body mt-5 max-w-[46ch] text-chalk-soft">
              The surface is cut into 999 parcels of equal ground, from
              Oceanus Procellarum to the farside nobody has ever seen from
              here. Every parcel has its own token and its own market, so one
              parcel can be held by hundreds of people at once — you
              included.
            </p>
