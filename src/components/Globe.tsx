 ...
 *
 * Every point on it — plot centres, plot corners, mare outlines, graticule —
 ...
 *
 * A plot with no market is an outline; one that trades burns amber in
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      for (const star of STARS) {
        ...
        ctx.fillStyle = `rgba(236, 232, 226, ${star.a})`;
        ctx.fill();
      }

      /*
       * No atmosphere, so no halo. An airless body has a hard edge against
       * the sky — the glow that sells a planet is exactly what would make
       * this one wrong. A tight rim, barely wider than the disc, keeps it
       * from disappearing into the page without pretending there is air.
       */
      ...
      rim.addColorStop(0, "rgba(226, 220, 210, 0.16)");
      rim.addColorStop(1, "rgba(226, 220, 210, 0)");
      ctx.beginPath();
      ctx.arc(f.cx, f.cy, radius * 1.05, 0, Math.PI * 2);
      ctx.fillStyle = rim;
      ctx.fill();

      // The body, lit from the upper left. Warm grey: regolith is not blue.
      ...
      body.addColorStop(0, "#332F2A");
      body.addColorStop(0.55, "#1E1C1A");
      ...
      limb.addColorStop(0, "rgba(6, 6, 8, 0)");
      limb.addColorStop(1, "rgba(6, 6, 8, 0.78)");
      ctx.beginPath();
      ctx.arc(f.cx, f.cy, radius, 0, Math.PI * 2);
      ctx.fillStyle = limb;
      ctx.fill();

      ctx.strokeStyle = "rgba(226, 220, 210, 0.42)";
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.arc(f.cx, f.cy, radius, 0, Math.PI * 2);
      ctx.stroke();
      ...
      ctx.lineWidth = 0.6;
      ctx.strokeStyle = "rgba(226, 220, 210, 0.08)";
      for (const line of graticule) strokePolyline(line, f, false);

      ctx.lineWidth = 0.8;
      ctx.strokeStyle = "rgba(226, 220, 210, 0.30)";
      for (const ring of mariaVectors) strokePolyline(ring, f, true);
       ...
       * to read curvature.
       *
       * The hexes tile the whole surface, so filling them is also what
       * draws the Moon's face: each band is laid down twice, once for the
       * basalt plains and once for the highlands, at different weights. The
       * maria are darker ground, so they take the lower alpha and fall
       * back; nothing else is needed to make the nearside recognisable.
       * There is no separate albedo layer, which would have to be clipped
       * to the visible hemisphere and would tear at the limb.
       */
      const bands = [
      ...
      /*
       * Multipliers on each band, basalt first. The spread has to be wide:
       * at a subtle setting the maria read as noise, and the whole point of
       * drawing the face out of the parcels is that the nearside should be
       * recognisable before a single label is read.
       */
      const TERRAIN = [
        { mare: 1, fill: 0.18, stroke: 0.42 },
        { mare: 0, fill: 3.2, stroke: 1.15 },
      ];

      for (let b = 0; b < bands.length; b += 1) {
        const band = bands[b];
        const max = b === 0 ? 2 : bands[b - 1].min;
        for (const tone of TERRAIN) {
        ctx.beginPath();
        for (let i = 0; i < parcels.length; i += 1) {
          if (parcelIsMare[i] !== tone.mare) continue;
        ...
        ctx.fillStyle = `rgba(232, 228, 220, ${band.fill * tone.fill})`;
        ctx.fill();
        ctx.strokeStyle = `rgba(232, 228, 220, ${band.stroke * tone.stroke})`;
        ctx.lineWidth = 0.7;
        ctx.stroke();
        }
      }
      ...
      for (let i = 0; i < parcels.length; i += 1) {
        ...
        // Mint once a plot has the most owners: at a glance, amber is a
      ...
      emphasise(selectedRef.current, "#ff9e2c", 2.2);
